...
SEASON_TYPES: dict[str, str] = {"REG": "regular season", "POST": "postseason"}
...
def get_processed_string_column(df: pd.DataFrame) -> pd.Series:
    ...
    df["summary"] = df.apply(
        lambda x: (
            f"{x['player_name']} was drafted in round {int(x['draft_round'])}, "
            ...
            f"{int(x['receiving_touchdown'])} receiving touchdowns."
        ),
        axis=1,
    )
